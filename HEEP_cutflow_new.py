...
class cutflow:
    ...
    def print_results_latex(self):
        file_name=self.name.replace("_total","")
        file_cutflow = open(str(file_name+'.txt'),'w+')
        file_cutflow.write( "\\begin{table}[htb]\n")
        caption_name=file_name.replace("_cutflow"," ")
        file_cutflow.write( "\\caption{Cutflow for "+caption_name+"\n")
        file_cutflow.write( "\\label{tab:"+file_name+"}}\n")
        file_cutflow.write( "\\begin{center}\n")
        file_cutflow.write( "\\begin{tabular}{rcccccc}\n")
        file_cutflow.write( "\\hline\n")
        file_cutflow.write( '%40s & %15s & %15s & %15s & %15s & %15s & %15s \\\\'%('', 'nEvents', 'cum', 'N-1', 'nElectrons', 'cum', 'N-1'))
        file_cutflow.write("\n")
        file_cutflow.write( '%40s & %15s & %15s & %15s & %15s & %15s & %15s \\\\'%('', '', 'nEvents', 'events', '', 'nElectrons', 'electrons'))
        file_cutflow.write("\n")


        file_cutflow.write( "\\hline\n")
        file_cutflow.write( '%40s & %15s & %15d & %15s & %15s & %15d & %15s \\\\'%('Raw', '', self.nEvents['raw'], '', '', self.nElectrons['raw'], ''))
        file_cutflow.write("\n")
        file_cutflow.write( "\hline\n"  )
        for cn in self.cut_names:
            latex_name=cn.replace(str(file_name+"_"),"")
            file_cutflow.write( '%40s & %15d & %15d & %15d & %15d & %15d & %15d'%(latex_name, self.nEvents[cn], self.cumulative_nEvents[cn], self.NM1_nEvents[cn], self.nElectrons[cn], self.cumulative_nElectrons[cn], self.NM1_nElectrons[cn]))
            file_cutflow.write("\\\\\n") #to write correctly \, you write \\. Since I need \\, i'll write \\\\

        file_cutflow.write( "\\hline\n"  )
        file_cutflow.write( "\\end{tabular}\n"  )
        file_cutflow.write( "\\end{center}\n"  )
        file_cutflow.write( "\\end{table}\n"  )
